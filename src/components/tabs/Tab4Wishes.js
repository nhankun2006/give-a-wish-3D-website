...
export default function Tab4Wishes({ isUnlocked, setIsUnlocked }) {
  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        // Normalize array elements to { id, edit_token }
        const normalized = parsed.map(item => typeof item === 'string' ? { id: item, edit_token: null } : item);
        setMyWishesIds(normalized);
      } catch (e) {
        setMyWishesIds([]);
      }
    }
  ...
  const handleDeleteWish = async (id, edit_token) => {
    if (!window.confirm("Bạn có muốn thu hồi lại bọt biển này không?")) return;
    try {
      const res = await fetch('/api/wishes', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id, edit_token })
      });
      if (!res.ok) {
        const errData = await res.json().catch(() => ({}));
        throw new Error(errData.error || 'Lỗi khi xóa!');
      }
      
      // Xóa ID khỏi danh sách của mình
      const updatedIds = myWishesIds.filter(wishObj => wishObj.id !== id);
      setMyWishesIds(updatedIds);
      localStorage.setItem('myWishesIds', JSON.stringify(updatedIds));
      
      fetchWishes();
    } catch (e) { 
      alert(e.message || "Lỗi khi xóa, vui lòng thử lại!"); 
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEditing) {
        const editWishObj = myWishesIds.find(w => w.id === editWishId);
        if (!editWishObj || !editWishObj.edit_token) {
          throw new Error('Bạn không có quyền sửa thư này (thiếu mã token).');
        }
        ...
        const res = await fetch('/api/wishes', {
          ...
          body: JSON.stringify({ 
            ...
            name: newName.trim(), 
            message: newMsg.trim(),
            edit_token: editWishObj.edit_token 
          }),
        });
        
        if (!res.ok) {
          const errData = await res.json().catch(() => ({}));
          throw new Error(errData.error || 'Sửa thất bại, thử lại nhé.');
        }
        setFormNote('Đã sửa lời chúc thành công!');
        fetchWishes(); // reload data
      } else {
        const res = await fetch('/api/wishes', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        const returnedData = dataJson.data;
        
        setFormNote('Tín hiệu đã gửi!');

        // Tự động tạo bức thư mới ở local để Canvas vẽ ngay lập tức
        setWishes(prevWishes => [returnedData, ...prevWishes]);
        setNewWishId(returnedData.id);
        
        const updatedIds = [...myWishesIds, { id: returnedData.id, edit_token: returnedData.edit_token }];
        setMyWishesIds(updatedIds);
        localStorage.setItem('myWishesIds', JSON.stringify(updatedIds));
      }
  ...
  return (
    ...
    >
      {!isUnlocked ? (
        ...
        <>
          {/* LỚP 1: VÙNG CANVAS - ĐƯỢC PHÉP CUỘN */}
          <div className="absolute inset-0 overflow-y-auto overflow-x-hidden scroll-smooth" style={{ background: 'rgba(2, 20, 40, 0.4)', backdropFilter: 'blur(3px)', zIndex: 1 }}>
          ...
          {/* LỚP 2: GIAO DIỆN NỔI - KHÔNG BAO GIỜ BỊ CUỘN */}
          <div className="absolute inset-0 pointer-events-none z-20">
            ...
            {/* Buttons & Hộp Quà */}
            <div className="absolute top-4 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 w-full max-w-[1200px]">
              <div style={{ display: 'flex', gap: '270px' }} className="pointer-events-auto">
                ...
                <button
                  ...
                  className="
                    ...
                    px-10 pt-3 pb-4 rounded-[2rem] border-[2px] border-[#ff99c4]
                ...
                >
                  ...
                  <span className="relative z-10 text-2xl group-hover:-rotate-12 group-hover:scale-110 transition-transform duration-300">💌</span>
                  <span className="relative z-10 drop-shadow-[0_1px_2px_rgba(255,255,255,0.9)] tracking-wide">Thả Bọt Biển</span>
                </button>
                ...
                <button
                  ...
                  className="
                    relative overflow-hidden flex items-center justify-center gap-3
                    px-10 pt-3 pb-4 rounded-[2rem] border-[2px] border-[#64d9ff]
                ...
                >
                  ...
                  <div className="absolute right-6 bottom-3 w-2 h-2 rounded-full bg-white/70" />
                  <span className="relative z-10 text-2xl group-hover:-rotate-12 group-hover:scale-110 transition-transform duration-300">📚</span>
                  <span className="relative z-10 drop-shadow-[0_1px_2px_rgba(255,255,255,0.9)] tracking-wide">Thư Đã Xem ({viewedWishes.length})</span>
                </button>
              </div>

              {/* Box Gửi Thư / Hộp Thư Của Bạn */}
                    ...
                    >
                      <div
                        ...
                        style={{
                          ...
                          borderRadius: '40px', padding: '24px 32px',
                          width: '980px', maxWidth: '92vw', height: '70vh', gap: '16px',
                          boxShadow: `
                            0 0 0 6px ${activeTab === 'create' ? '#64d9ff' : '#ff99c4'}, 
                            0 25px 0 ${activeTab === 'create' ? '#007099' : '#b34774'},
                            0 40px 50px rgba(0,0,0,0.5), inset 0 10px 30px rgba(255,255,255,0.2)
                          `
                        }}
                      >                    
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 10 }}>
                          <span style={{ 
                            ...mono, fontSize: 20, color: '#FFF', fontWeight: '900', textTransform: 'uppercase', letterSpacing: 2,
                        ...
                        <div style={{ display: 'flex', gap: '8px', zIndex: 10, marginTop: '-6px' }}>
                          <button onClick={() => { setActiveTab('create'); if (!isEditing) { setNewName(''); setNewMsg(''); } }}
                        ...
                        <div style={{ zIndex: 10, flex: 1, display: 'flex', flexDirection: 'column', height: '100%', overflowY: 'hidden', overflowX: 'hidden' }}>
                          {activeTab === 'create' ? (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', height: '100%', flex: 1, overflow: 'hidden' }}>      
                              ...
                              <textarea value={newMsg} onChange={e => setNewMsg(e.target.value)} placeholder="Hãy viết điều gì đó dễ thương vào đây nhé... 🐚" 
                                ...
                                style={{ 
                                  ...
                                  border: '4px solid #64d9ff', color: '#1A548B', fontSize: 16, fontWeight: '600', padding: '20px 24px', 
                                  outline: 'none', width: '100%', resize: 'none', lineHeight: 1.6, boxShadow: 'inset 0 4px 8px rgba(0,0,0,0.08)', overflowY: 'auto'
                                }} />
                              <button onClick={handleSubmit} disabled={isSubmitting}
                                className={`
                                  relative flex items-center justify-center gap-3 pt-3.5 pb-3.5 mt-1 rounded-[24px] 
                                  font-black text-[18px] transition-all duration-150 cursor-pointer w-full flex-shrink-0
                            ...
                            <div className="cute-scroll" style={{ display: 'flex', flexDirection: 'column', gap: '16px', flex: 1, overflowY: 'auto', maxHeight: '100%', paddingRight: '12px', paddingBottom: '20px' }}>
                              {wishes.filter(w => myWishesIds?.some(item => item.id === w.id)).length === 0 ? (
                                ...
                                wishes.filter(w => myWishesIds?.some(item => item.id === w.id)).map((wish) => {
                                  const wishMeta = myWishesIds.find(item => item.id === wish.id);
                                  const canEdit = !!(wishMeta && wishMeta.edit_token);

                                  return (
                                    <div key={wish.id} style={{ 
                                      background: '#FFF', border: '4px solid #ff99c4', borderRadius: '24px', padding: '20px',
                                      boxShadow: '0 6px 0 #b34774', transform: 'translateY(0)', transition: '0.2s ease-in-out', flexShrink: 0 
                                    }}
                                      ...
                                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                                        ...
                                        {canEdit && (
                                          <div style={{ display: 'flex', gap: '10px' }}>
                                            ...
                                            <button onClick={() => handleDeleteWish(wish.id, wishMeta.edit_token)}
                                              className="active:translate-y-1 active:shadow-none transition-all"
                ...
                {historyOpen && (
                  <motion.div
                    ...
                    style={{
                      ...
                      borderRadius: '24px', background: 'linear-gradient(180deg, #81D4FA 0%, #B2EBF2 60%, #FFF9C4 100%)', 
                      border: '4px solid #FFFFFF', boxShadow: '0 15px 35px rgba(2, 119, 189, 0.25)',
                    }}
                  >
                    <style>{`.cute-scroll::-webkit-scrollbar { display: none; }`}</style>
                    ...
                    <div className="cute-scroll" style={{ overflowY: 'auto', maxHeight: 'cal(85vh - 180px)', display: 'flex', flexDirection: 'column', gap: '12px', scrollbarWidth: 'none', msOverflowStyle: 'none', padding: '4px' }}>
                      {viewedWishes.filter(w => w.name.toLowerCase().includes(searchTerm.toLowerCase()) || w.message.toLowerCase().includes(searchTerm.toLowerCase())).map((wish, index) => (
                        <motion.div key={wish.id} 
                          ...
                          style={{ background: '#FFFFFF', border: '2px solid #E1F5FE', borderRadius: '16px', padding: '16px', cursor: 'pointer', transition: 'all 0.1s ease-out' }}
                        >
                          <div style={{ fontSize: '15px', color: '#0277BD', fontWeight: '900', marginBottom: '6px' }}>💌 {wish.name}</div>
                      ...
                      {viewedWishes.filter(w => w.name.toLowerCase().includes(searchTerm.toLowerCase()) || w.message.toLowerCase().includes(searchTerm.toLowerCase())).length === 0 && (
                        <div style={{ textAlign: 'center', color: '#01579B', marginTop: '20px', fontSize: '14px', fontWeight: '700' }}>Không tìm thấy thư nào...</div>
                      )}
                    </div>
                  </motion.div>
                )}
            ...
            {/* COMPONENT ĐỌC THƯ MỞ RA GIỮA MÀN HÌNH TẠI ĐÂY */}
            <div className="pointer-events-auto">
              <WishReader 
